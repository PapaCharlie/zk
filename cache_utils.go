...
func getNodeData(policy RetryPolicy, stopChan chan struct{}, nodePath string, conn *Conn) (data []byte, stat *Stat, err error) {
...
func getNodeChildren(policy RetryPolicy, stopChan chan struct{}, nodePath string, conn *Conn) (children []string, stat *Stat, err error) {
	err = ExecuteWithRetries(policy, stopChan, func() (err error) {
		children, stat, err = conn.Children(nodePath)
		return err
	})
	return children, stat, err
}

func getNodeDataAndChildren(policy RetryPolicy, stopChan chan struct{}, nodePath string, conn *Conn) (data []byte, stat *Stat, children []string, err error) {
	err = ExecuteWithRetries(policy, stopChan, func() (err error) {
		data, stat, children, err = conn.GetDataAndChildren(nodePath)
		return err
	})
	return data, stat, children, err
}
